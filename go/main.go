...
func postInitialize(c echo.Context) error {
	...
	// set condition_level of initial data
	conditions := make([]IsuCondition, 0, 1000)
...
func getIsuConditionsFromDB(db *sqlx.DB, jiaIsuUUID string, endTime time.Time, conditionLevel []string, startTime time.Time,
	...
	if len(conditionLevel) > 0 {
		levelFilter = fmt.Sprintf("	AND `condition_level` IN (%s)", "'"+strings.Join(conditionLevel, "', '")+"'")
	}
...
func GetTrendByte() ([]byte, error) {
	...
	query := `
		SELECT isu.id AS isu_id, isu.character, isu_condition.condition_level, isu_condition.timestamp
		FROM isu
		...
		ORDER BY isu_condition.timestamp DESC
	`
	...
	for _, res := range queryRes {
		...
		trendResp, ok := respByCharacter[res.Character]
		if !ok {
			trendResp = TrendResponse{
		...
		case "info":
			trendResp.Info = append(trendResp.Info, &trendCondition)
		case "warning":
			trendResp.Warning = append(trendResp.Warning, &trendCondition)
		case "critical":
			trendResp.Critical = append(trendResp.Critical, &trendCondition)
		}
		respByCharacter[res.Character] = trendResp
	}
	...
	for _, character := range characterList {
		resps, ok := respByCharacter[character.Character]
		if !ok {
			resps = TrendResponse{
		...
		res = append(res,
			TrendResponse{
				Character: character.Character,
				Info:      resps.Info,
				Warning:   resps.Warning,
				Critical:  resps.Critical,
			})
	}
